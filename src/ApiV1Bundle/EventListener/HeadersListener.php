...
class HeadersListener
{
    ...
    private function checkRoute($pathInfo, $routes, $method)
    {
        foreach ($routes as $path => $data) {
            $subroute = substr($pathInfo, 0);
            if ($this->pathRegExp($path, $subroute)) {
    ...
    private function pathRegExp($path, $subroute)
    {
        $regexp = str_replace('/', '\/', $path);
        $regexp = '/^' . $regexp . '$/si';
        $matches = [];
        preg_match_all($regexp, $subroute, $matches, PREG_SET_ORDER, 0);
