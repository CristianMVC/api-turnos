...
/**
 * Class UserOrganismoValidator
 * @package ApiV1Bundle\Entity\Validator
 */
class UserOrganismoValidator extends UserValidator
{
    /**
     * UserOrganismoValidator constructor.
    ...
    /**
     * Validar parametros para la creación de un usuario organismo
     *
    ...
    /**
     * Validamos parametros para la edición de un user organismo
     *
     * @param array $params array con datos a validar
     * @param UserOrganismo $userOrganismo
     * @param integer $id Identificador único de usuario
     * @return ValidateResultado
     */
    public function validarEdit($params, $userOrganismo, $id = null)
    {
        $validateResultado = $this->validarDatosBasicos($params, $id);
        if (! $validateResultado->hasError()) {
            $validateResultado = $this->validarEntidad($userOrganismo, 'Usuario inexistente');
        }
        if (! $validateResultado->hasError()) {
