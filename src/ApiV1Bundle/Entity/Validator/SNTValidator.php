...
class SNTValidator
{
    ...
    const STRING = 'El campo no puede contener números';
    const DURACION = 'La duración del tramite es incorrecta, debe ser mayor a 5 y menor a 480';
    const ETIQUETA_REQUERIDA = 'La etiqueta es requerida';
    ...
    /*
     * Me chequea la hora en forma = HH:MM
    */
    private function timeTryCatch($campos, $key)
    {
        $format = 'H:i';
        $time = $campos[$key];
        try {
     ...
     * @return string|NULL
     */
    private function timeHis($campos, $key)
    {
        $format = 'H:i:s';
        $time = $campos[$key];
        $d = new \DateTime(trim($time));
    ...
    /**
     * Valida la duracion del turno
     *
     * @param type $campos
     * @param type $key
     * @return type
     */
    private function duracionTramite($campos, $key)
    {
        if ($campos[$key] < 5 || $campos[$key] > 480) {
    ...
    /**
     * Validar si los campos del formulario tienen etiqueta
     *
     * @param mixed $valor
     * @return string|NULL
     */
    private function etiqueta($campos, $key)
    {
        if (! isset($campos[$key]) || (empty($campos[$key]) && (string)$campos[$key] != '0')) {
            return self::ETIQUETA_REQUERIDA;
        }
        return null;
    }
}
